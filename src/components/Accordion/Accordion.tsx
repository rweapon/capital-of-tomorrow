        ...
        <section className="mx-auto w-full max-w-[1440px] px-4 sm:px-10 lg:px-24">
            ...
            <div className=" px-2 sm:px-4 lg:px-[34px]">
                {items.map((item, index) => (
                    <article
                        key={index}
                        className={`border-b border-white/20 p-9 ${index === 0 ? 'border-t' : ''}`}
                    >
                        <button
                            className={`flex w-full items-center justify-between text-left transition-colors ${activeIndices.includes(index)
                                    ? 'text-white'
                        ...
                        >
                            ...
                            <svg
                                className={`size-6 transition-transform ${activeIndices.includes(index) ? 'rotate-180' : ''}`}
                                fill="none"
                                stroke="currentColor"
                                viewBox="0 0 24 24"
                        ...
                        <div
                            ...
                            className={`overflow-hidden pt-2 transition-all duration-300 ${activeIndices.includes(index) ? 'max-h-96' : 'max-h-0'
                                }`}
                        >
