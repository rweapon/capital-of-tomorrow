...
const Card: React.FC<ICardProps> = ({
  ...
  const renderBackground = () => {
    switch (backgroundType) {
      ...
      default:
        return <path d={path} fill={backgroundColor} stroke={borderColor} strokeWidth="1" />;
    }
  };
          ...
          <header className="flex min-h-[256px] flex-col justify-between px-[34px] py-8">
            ...
            {price && (
              <div className="-ml-24 text-center leading-[34px]">
                ...
                <span className="font-akira text-[48px] font-[800]" style={textStyle}>
                  {price}
                </span>
              </div>
            )}
            ...
            <ul className="font-mont text-[13px] leading-none ">
              {listItems.map((item, index) => (
                <li key={index} className="flex items-start">
                  <span className="mr-3 mt-1 text-xs leading-none text-[#E3AF64]">•</span>
  ...
  return (
    <div className="relative " style={{ width, height, filter: `drop-shadow(${shadow})` }}>
      ...
      <div className="absolute inset-0 flex flex-col" style={{ clipPath: `path('${path}')` }}>
        {renderContent()}
      </div>
    </div>
  );
