        ...
        <section className="mx-7 flex flex-col gap-3 sm:gap-5 md:gap-7 lg:gap-8 xl:mx-[124px] xl:gap-10">
            ...
            <ul className="flex flex-col gap-2 md:flex-row md:gap-4 lg:gap-5 xl:gap-7">
                {timelineContent.events.map((event, index) => (
                    <li
                        key={index}
                        className={`rounded-lg bg-[#F8F7F5]/[0.05] shadow-[inset_0_0_15px_0_rgba(0,0,0,0.1)] ${index == 1 ? 'text-[#E3AF64]' : 'text-[#F8F7F5]'}`}
                    >
                        <header
                            className={`flex flex-col border-b border-dotted ${index === 1 ? 'border-[#E3AF64]' : 'border-[#F8F7F5]'} p-5 sm:p-6 md:p-8 lg:p-7 xl:px-8 xl:pb-8 xl:pt-10`}
                        >
