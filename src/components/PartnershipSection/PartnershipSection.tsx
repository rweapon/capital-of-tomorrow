        ...
        <div className="relative flex min-h-[400px] flex-col sm:min-h-[450px] md:min-h-[500px] lg:h-[500px] lg:flex-row-reverse">
          <div
            ...
            style={{
              ...
              maskImage: 'radial-gradient(ellipse at 79% 30%, black 25%, transparent 68%)',
              WebkitMaskImage: 'radial-gradient(ellipse at 79% 30%, black 25%, transparent 68%)',
              ...
              <ul className="space-y-0">
                {partnershipContent.advantages.map((item, index) => (
                  <li key={index}>
                    ...
                    <p
                      className={`font-mont ${index != 3 ? 'mb-[14px] border-b border-[#F8F7F5]/75 pb-[14px]' : ''} text-[13px] font-normal leading-4 text-white/80`}
                    >
                      {item.text}
                    </p>
                  </li>
        ...
        <div className="mt-[74px] w-full px-[50px] pb-12">
          <div className="flex items-center justify-between gap-4">
            {partnerLogos.map((logo, index) => (
              <div key={index} className={`hh- flex flex-1 justify-center ${logo.height}px`}>
                <Image
                  src={`/${logo.type}.svg`}
                  alt="Partner logo"
                  width={logo.height * 2}
